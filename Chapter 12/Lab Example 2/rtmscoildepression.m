...
f_alpha = 10; % Frequency of alpha oscillations (8-12 Hz)
f_theta = 6; % Frequency of theta oscillations (4-7 Hz)
p0_alpha = [1, 0, 0]; % Initial dipole moment for alpha (arbitrary units)
p0_theta = [0.5, 0, 0]; % Initial dipole moment for theta (arbitrary units)
 
 
% Calculate baseline MEG signal with altered oscillatory patterns
meg_baseline_alpha = mu0 * (3 * (p0_alpha(1) * z) .* z - p0_alpha(1)) ./ (4 * pi * (x.^2 + y.^2 + z.^2).^(5/2));
meg_baseline_theta = mu0 * (3 * (p0_theta(1) * z) .* z - p0_theta(1)) ./ (4 * pi * (x.^2 + y.^2 + z.^2).^(5/2));
meg_baseline = meg_baseline_alpha + meg_baseline_theta; % Combine alpha and theta components
...
meg_post_alpha = mu0 * (3 * (1.2 * p0_alpha(1) * z) .* z - 1.2 * p0_alpha(1)) ./ (4 * pi * (x.^2 + y.^2 + z.^2).^(5/2));
meg_post_theta = mu0 * (3 * (1.2 * p0_theta(1) * z) .* z - 1.2 * p0_theta(1)) ./ (4 * pi * (x.^2 + y.^2 + z.^2).^(5/2));
meg_post = meg_post_alpha + meg_post_theta; % Combine alpha and theta components
...
for i = 1:length(t)
    ...
    meg_baseline_time(i, :) = sin(2 * pi * f_alpha * t(i)) * meg_baseline(num_points/2, num_points/2, num_points/2);
    meg_post_time(i, :) = sin(2 * pi * f_alpha * t(i)) * meg_post(num_points/2, num_points/2, num_points/2);
end
